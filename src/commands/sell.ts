	...
	async run(message: KlasaMessage, [member, quantity, itemName, price]: [KlasaUser, number, string, number]) {
        ...
        try {
            ...
            return message.sendMessage(
                `*Your sale request has been canceled. Please try again.*`
            )
        }

        let provider = this.client.providers.get("airtables") as airtables;
        let resp = await provider.sell(transaction);

        let response = `You sold **${transaction.fields.Quantity.toLocaleString()}** x **${transaction.fields.Item}** for **${transaction.fields.SellPrice.toLocaleString()}** each${message.author.id !== trader.id ? " on behalf of <@" + trader.id.toString() + ">" : ""} and made **${resp.profit.toLocaleString()}**!`;
